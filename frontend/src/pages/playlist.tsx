...
export function Playlist() {
  ...
  const { data: playlistData } = useQuery({
    ...
    queryFn: async () => {
      ...
      const result = await execute(PlaylistQuery, { id: playlistId });
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["playlist-tracks", playlistId, page, pageSize],
    queryFn: async () => {
      if (!playlistId) throw new Error("Playlist ID is required");
      ...
      return {
        ...
        tracks: result.playlistTracks.tracks.map((track) => ({
          ...track,
  ...
  if (!playlistId) {
    return (
      <div className="container mx-auto p-8">
        <div className="text-muted-foreground">Invalid playlist ID</div>
      </div>
    );
  }
